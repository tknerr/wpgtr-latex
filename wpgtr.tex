...
\usepackage[papersize={8.125in,10.250in},bindingoffset=1cm,left=2cm,right=2cm,top=3cm,bottom=2cm]{geometry}
\geometry{twoside}
\usepackage{setspace}
...
\newminted{console}{fontsize=\small,style=pastie}
\newmintinline{ruby}{fontsize=\small}
\newmintinline{console}{fontsize=\small,style=pastie}

% pseudo-mint for plain bold black formatting
...
\ % padding
	...
	\hspace{2.3cm}\includegraphics[width=0.3575\textwidth]{cache/8.png}
...
\ % fix formatting with an emtpy char here
...
\ % fix formatting
	...
	\hspace{2.3cm}\includegraphics[width=0.3575\textwidth]{cache/9.png}
...
\ % padding
	...
	\hspace{2.3cm}\includegraphics[width=0.3575\textwidth]{cache/10.png}
...
\ % padding
	...
	\hspace{8.3cm}\includegraphics[width=0.3575\textwidth]{cache/11.png}

\ % fix formatting with this magic whitespace
...
\ % padding
	...
	\hspace{5cm}\includegraphics[width=0.3575\textwidth]{cache/12.png}
...
and not just one changes, they all change.
	...
	\hspace{5cm}\includegraphics[width=0.3575\textwidth]{cache/13.png}
...
Variables are passed through this chute (or tunnel) into the block.
	...
	\hspace{5cm}\includegraphics[width=0.3575\textwidth]{cache/14.png}
...
\ \unskip{\bf Say aloud each of the parts of speech used below.}
...
\begin{center}
	...
	\includegraphics[width=1.0\textwidth]{cache/23.png} 
...
{\plaininline String} class.  Which means that {\bf
...
I went ahead and saved the Hash of code words to a file called {\bf\plaininline wordlist.rb}.
...
named {\bf\plaininline wordlist.rb}.
...
word, handing each pair to the {\plaininline gsub!}
method for the actual replacement.

In Ruby, {\plaininline gsub} is short for {\em global
...
its safe code word.  With {\plaininline gsub}, you
...
{\plaininline gsub}?  Doesn't
{\plaininline gsub} give us an answer back that we
...
Yes, with {\plaininline gsub} we'd need to hang on to
its answer.  We're using a variation of
{\plaininline gsub} that is totally hyper.  Notice the
{\bf exclamation mark} on the {\plaininline gsub!} used
inside the \rubyinline|each| block.  The exclamation
mark is a sign that {\plaininline gsub!} is a bit of a
zealot.  See, {\plaininline gsub!} will go ahead and
...
Call {\plaininline gsub!} a {\bf destructive method}.
It makes its changes to the value directly.  Whereas
{\plaininline gsub} will leave the value intact,
answering back with a new string which contains the alterations. (Why
must {\plaininline gsub!} scream when he descends upon
...
{\plaininline Array} the Caterpillar, with a
{\plaininline String} for each file.  If you are curious
...
Pulls an error: 
\consoleinline|undefined local variable or method `verb'|. Poof.
...
file {\bf\plaininline endertromb.rb}, which is loaded with
...
For a while he knocked.  Which paid off.
	...
	\includegraphics[width=1.0\textwidth]{cache/38.jpg}
...
{\plaininline String}, \rubyinline|Dir|,
\rubyinline|LoadError|,
{\plaininline Float}.  Each of those came with Ruby.
...
\pagebreak
	...
	\begin{sidebar}{An Evening of Unobstructed Voltage}{320}
...
found myself wandering through hallways,\unskip\parfillskip 0pt \par}

\pagebreak

	\begin{sidebar}{An Evening of Unobstructed Voltage}{40}
		\textit{(cont'd)}
...
is telling us that there is no {\plaininline include?}
...
{\plaininline include?} are methods which work with
...
Numbers have no {\plaininline include?} method.  Great,
...
The {\plaininline respond\_to?} method is really nice and
I plead that you never forget it's there.  The
{\plaininline respond\_to?} checks any object to be sure
...
{\plaininline include?} method.  If no
{\plaininline include?} method is found, then we raise
the error.

You might be wondering why I used a symbol with
{\plaininline respond\_to?}.  I used a symbol
\rubyinline|:include?| instead of a string
\rubyinline|'include?'|.  Actually, either will work
with {\plaininline respond\_to?}.
...
catches the eye.  The {\plaininline respond\_to?} asks
...
Yes, absolutely, it should!  {\bf It's bad manners to change strings
  like that.}  We've used {\plaininline gsub} and
{\plaininline gsub!} in the last chapter.  Do you
...
The {\plaininline dup} method makes a copy of any
...
it disappear?
	...
	\begin{sidebar}{An Excerpt from The Scarf Eaters}{50}
		\vspace{6pt}
...
Oh, and one more thing about {\plaininline dup}.  Some
...
\item It's poor etiquette to change objects your method is given.  Use
  {\plaininline dup} to make a copy.  Or find a method
  like {\plaininline gsub} that automatically makes a
  copy as it does its job.
\item The square brackets can be used to lookup parts inside any
  {\plaininline Array},
  \rubyinline|Hash| or
  {\plaininline String} objects, as these objects
...
{\plaininline String}, {\bf one of the core classes of
  Ruby}!
...
{\plaininline String} class?  Two things: a class
...
\pagebreak
	...
	\begin{sidebar}[0.48]{\vspace*{-33pt}}{10}
\vspace{6pt}
\begin{verbatim}
...
variation to the {\plaininline Array} class. You want to
...
{\plaininline Array} class with its own
\rubyinline|join| method.
{\plaininline Array} is the {\bf superclass} of
...
{\plaininline Array} of our room sizes:
...
I get an error: 
\consoleinline|undefined method `picks='|. 
...
unless they are put in a machine that uses video cassettes. They depend
...
you as a {\plaininline String}.  You can also read the
...
{\bf\plaininline preeventualist.rb}.
...
\ \unskip {\bf the program jumping onto your
...
of \rubyinline|Creature|.  {\bf Save this code into a
  file called {\bf\plaininline dwemthy.rb}.}
...
{\bf Add these lines to {\bf\plaininline dwemthy.rb}.}
...
{\bf\plaininline rabbit.rb} alters a few math symbols which
...
{\plaininline Array} and, thus, behaves just like one.
...
method in the {\plaininline String} class. This method
...
The \rubyinline|%|{\plaininline p} placeholder will run
...
\consoleinline|ri sprintf| page.  I'm just going to give
...
The {\plaininline clone} method makes an exact copy of a
...
The {\plaininline clone} method doesn't make copies of
...
You may also see the {\plaininline dup} method used to
copy objects.  The {\plaininline dup} method makes
copies which aren't as exact.  For example, there are objects in Ruby
which are ``frozen'' and can't be altered.  If you
{\plaininline clone} the object, you get an exact copy
which is also frozen.  If you use {\plaininline dup},
you get an unfrozen copy that you can change if you like.

The {\plaininline clone} method also copies the
metaclass of an object, whereas {\plaininline dup} does
not.
...
\\ {\plaininline \string\d} & matches digits & can also be
written {\plaininline [0-9]}
\\ {\plaininline \string\w} & matches word characters (letters,
numbers and the underscore) & can also be written
{\plaininline [A-Za-z0-9\_]}
\\ {\plaininline \string\s} & matches whitespace (spaces, tabs,
carriage returns, line feeds) & a.k.a. 
{\plaininline [\string\t\string\r\string\n]} \\ 
{\plaininline \string\D} & matches everything
{\em but} digits & a negated set {\plaininline [\string^\string\d]}
\\ {\plaininline \string\W} & matches everything {\em but} word
chars & just like {\plaininline [\string^\string\w]}
\\ {\plaininline \string\S} & matches everything {\em but}
whitespace & also {\plaininline [\string^\string\s]}
\\ {\plaininline .} & the period matches {\bf
  everything}. \\
\end{tabular}
...
\\ {\plaininline \{n\}} & match exactly {\em n} times &
Precisely three numbers in a row is
\rubyinline|/\d{3}/|
\\ {\plaininline \{n,\}} & matches {\em n} times or {\em
  more} & Three or more letters in a row is
\rubyinline|/[a-z]{3,}/i|
\\ {\plaininline \{n,n2\}} & matches at least {\em n}
...
\\ {\plaininline *} & the asterisk
{\plaininline *} is short for
{\plaininline \{0,\}} & To match a colon, followed by zero
or more word characters: \rubyinline|/:\w*/|
\\ {\plaininline +} & the plus is short for
{\plaininline \{1,\}} & To match one or more minus or plus
signs, use \rubyinline|/[-+]+/|
\\ {\plaininline ?} & the question mark is short for
{\plaininline \{0,1\}} & To match three numbers followed
...
The {\plaininline gsub} method is named for ``global
...
have a simple {\plaininline sub} method which will
substite only once.
...
\begin{minipage}{0.27\textwidth}
	\vspace*{-320pt}
	\parpic[l]{\includegraphics[width=0.92\textwidth]{cache/quil-2.png}}
\end{minipage}
...
was sleeping in a sleeping bag on my porch.\smallskip

She had run out of money to go to art school, so she stayed at my
place for five months or so.\smallskip
...
killed this kid on his tennis team \unskip\parfillskip 0pt \par}

\end{minipage}
\begin{minipage}{0.27\textwidth}
	\vspace*{-320pt}
	\parpic[r]{\includegraphics[width=0.92\textwidth]{cache/quil-1.png}}
\end{minipage}

and all these animals end up helping him figure it out.

She was writing a book about this kid who
...
I got a kick out of hearing each night.
	...
	\parpic[r]{\includegraphics[width=0.7\textwidth]{cache/quil-3.png}}
...
For instance, Ruby 1.8.3. To unpack and compile:
...
code. You may need to use \consoleinline|su| or \consoleinline|sudo| to give you permission to run the
\consoleinline|make install|.)
...
and select \consoleinline|Run...|. Type in: \consoleinline|cmd|. Press OK. A command shell window will
appear.
...
example computes the number of seconds between \rubyinline|now| and July 31, 2003
...
Notice how the depth level increased to 1 when I opened the \rubyinline|if|
...
senses. Try \newline \consoleinline|irb --prompt simple|. This time Irb will treat you to a
...
from inside of itself. The \rubyinline|conf| object
...
\emph{Enter}. The \rubyinline|%|{\plaininline 3n} describes that Irb should reserve three characters for
the line number. The \rubyinline|%l| saves a place for displaying the type of
...
the world here, folks, but the volume comes down from the sky---a
...
\bf\large (Ruby's Own 411 or 555-1212 or \emph{Yes, Operator, Get
    Belgrade on the Line}---I'll Be Right Here---Just Plain
...
It's an unabridged Ruby dictionary servo---the Power of Just Asking is
